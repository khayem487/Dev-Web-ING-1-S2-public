...
- [~] **P0 Setup baseline** (baseline validée; UI métier en cours)
  ...
  - [x] Install Tailwind CSS in frontend (deferred until baseline confirmed green)
  - [x] Switch from H2 to MySQL and add initial schema/seed (after first entities exist)
...
- [x] **P9 Administration (scope réintégré)**
  ...
  - [x] Journal admin plus lisible (motif décision + timestamp + auteur).

- [x] **Gap list vs repo de référence `cyZ-tech2/SmartHouse`**
...
- [x] **P7 Automation réactive + énergie + notifications**

  **Objectif** : faire vivre la maison de façon réactive (mouvement → action) et fournir au prof un dashboard énergétique crédible.

  ### Backend
  - [x] **P7.1** Étendre `Scenario.type` avec `CONDITIONAL` + champ `triggerObjetId` + `triggerEvent` (ex: `MOTION_DETECTED`, `BATTERY_LOW`, `TEMP_BELOW`).
  - [x] **P7.2** `POST /api/gestion/objets/{id}/simulate-event` `{ event: "MOTION_DETECTED" }` : déclenche l'événement (pas de capteur réel, donc bouton démo). Le service trouve les scénarios CONDITIONAL liés et les exécute.
  - [x] **P7.3** Évaluation contextuelle simple : un scénario CONDITIONAL peut avoir une `condition` libre (string → eval naïf : `night`, `day`, `temp<18`). Pour la démo : `night` = heure ∈ [20:00, 7:00].
  - [x] **P7.4** `GET /api/gestion/energie` : agrège `consoEnergie` des `Appareil` actifs, retourne `{ consoTotaleKwh, parPiece[], topConsommateurs[] }`.

  ### Frontend
  - [x] **P7.5** Bouton "Simuler événement" dans `DetailDrawer` pour `Camera` / `DetecteurMouvement` → POST `/simulate-event` → toast + scénario qui suit s'exécute visiblement.
  - [x] **P7.6** Nouvelle carte "Consommation" sur HomePage (KPI live) + section détaillée dans Gestion (top 3 consommateurs, barre par pièce).
  - [x] **P7.7** Slider de température cible pour Thermostat dans `DetailDrawer` (réutilise PUT `/objets/{id}` avec un nouveau champ `tempCible`).
  - [x] **P7.8** Notifications toast in-app pour : scénario exécuté, batterie < 20%, mouvement détecté. Polling léger toutes les 30s sur un nouvel endpoint `GET /api/gestion/notifications?since=…`.

- [x] **P8 Élargissement UML backend** (volume mécanique, faible créativité — à faire après P6/P7)
  - [x] Ajouter les entités manquantes : `Fenetre`, `PorteGarage`, `Climatiseur`, `Alarme`, `DetecteurMouvement`, `MachineCafe`, `Enceinte`, `Aspirateur`, `Arrosage`, `Reveil`, `SecheLinge`, `LaveVaisselle`.
  - [x] Étendre `GestionController#buildByType` pour chacun.
  - [x] Aligner `GESTION_TYPE_OPTIONS` côté front avec la liste backend.
  - [x] Endpoint `GET /api/gestion/historique?objetId={id}` pour filtrer côté serveur (le front filtre actuellement en mémoire).
  - [x] Mettre à jour `DataSeeder` pour seeder au moins 1 instance de chaque nouveau type (utile pour tester les scénarios "Sécurité" avec une vraie Alarme).
...
- [x] **DB r�elle MySQL d�ploy�e localement**
  - docker stack: `docker-compose.mysql.yml`
  - backend profile: `mysql`
  - URL backend valid�e: `http://localhost:8080/api/health`
  - front valid�: `http://localhost:5173`

- [x] **UX profil d�plac�e en sidebar**
  - �dition profil via clic carte profil bas-gauche (modale d�di�e)
  - formulaire retir� de la page Mes objets

- [x] **Onboarding compte dev non bloquant**
  - `app.auth.email-verification-enabled=false` par d�faut
  - compte nouvellement cr�� utilisable imm�diatement sans SMTP
